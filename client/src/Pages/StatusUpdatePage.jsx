...
export default function StatusUpdatePage() {
  ...
  const handleStatusUpdate = (userId, status, event) => {
    ...
    setConfirmModal({
      open: true,
      title:
        status === "Active" ? "Zaakceptuj użytkownika?" : "Odrzuć użytkownika?",
      message: `Czy na pewno chcesz ${
        status === "Active" ? "zaakceptować" : "odrzucić"
      } użytkownika?`,
      onConfirm: async () => {
  ...
  const handleMarkAllAccepted = (userId) => {
    ...
    setBatchedPhotoUpdates((prev) => {
      const otherUpdates = prev.filter(
        (p) => !photos.some((photo) => photo.id === p.id)
      );
      const newUpdates = photos.map((photo) => ({
        id: photo.id,
        photoStatus: "Active",
      }));
  ...
  const handleBatchPhotoUpdate = (photoId, status) => {
    setBatchedPhotoUpdates((prev) => {
      ...
      if (existing) {
        ...
        return prev.map((p) =>
          p.id === photoId ? { ...p, photoStatus: status } : p
        );
      } else {
  ...
  const handleSubmitAllPhotoUpdates = () => {
    ...
    setConfirmModal({
      ...
      message:
        "Czy na pewno chcesz zatwierdzić wszystkie zaznaczone zmiany zdjęć?",
      onConfirm: async () => {
        setConfirmModal((prev) => ({ ...prev, open: false }));
        try {
          ...
          setUserPendingPhotos((prev) => {
            ...
            batchedPhotoUpdates.forEach((p) => {
              Object.keys(newPhotos).forEach((uid) => {
                newPhotos[uid] = newPhotos[uid].filter(
                  (photo) => photo.id !== p.id
                );
              });
            });
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6 border-b border-gray-200 flex gap-4 ">
          <button
            ...
            className={`pb-4 px-4 font-medium hover:cursor-pointer transition-colors ${
              activeTab === "users"
                ? "text-[#EA1A62] border-b-2 border-[#EA1A62]"
                : "text-gray-500 hover:text-gray-700"
            }`}
          >
            <div className="flex items-center gap-2 ">
              <Users size={20} /> Użytkownicy
              <span className="bg-[#EA1A62] text-white px-2 py-1 rounded-full text-xs ">
                {pendingUsers.length}
              </span>
            </div>
          </button>
          ...
          (pendingUsers.length === 0 ? (
            <Card className="p-8 text-center">
              ...
              <p className="text-gray-500">
                Wszystkie konta zostały już zatwierdzone lub odrzucone.
              </p>
            </Card>
          ) : (
                      ...
                      <div className="flex-1 p-6">
                        <div className="flex flex-col md:flex-row md:items-start md:justify-between">
                          ...
                          <div className="flex flex-row md:flex-col gap-2 mt-4 md:mt-0 md:ml-6">
                            ...
                            <Button
                              onClick={(e) =>
                                handleStatusUpdate(user.id, "Active", e)
                              }
                              variant="primary"
                              size="sm"
                            ...
                            <Button
                              onClick={(e) =>
                                handleStatusUpdate(user.id, "Rejected", e)
                              }
                              variant="primary"
                              size="sm"
                              disabled={isProcessing}
        ...
        {activeTab === "photos" &&
          (usersWithPhotos.length === 0 ? (
            <Card className="p-8 text-center">
              ...
              <h2 className="text-2xl font-semibold mb-2 text-gray-600">
                Brak oczekujących zdjęć
              </h2>
              <p className="text-gray-500">
                Wszystkie zdjęcia zostały już zatwierdzone lub odrzucone.
              </p>
            </Card>
          ) : (
            <>
                      ...
                      {photos.map((photo) => (
                        ...
                        >
                          ...
                          <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-60 transition-all flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                            <Button
                              onClick={() =>
                                handleBatchPhotoUpdate(photo.id, "Active")
                              }
                              size="sm"
                              className="flex items-center bg-green-600 hover:bg-green-700"
                            >
                            ...
                            <Button
                              onClick={() =>
                                handleBatchPhotoUpdate(photo.id, "Rejected")
                              }
                              size="sm"
                              className="flex items-center bg-red-600 hover:bg-red-700"
                            >
              ...
              <div className="mt-6 flex justify-end">
                <Button
                  onClick={handleSubmitAllPhotoUpdates}
                  size="md"
                  className="bg-blue-600 hover:bg-blue-700"
                  disabled={batchedPhotoUpdates.length === 0}
                >
