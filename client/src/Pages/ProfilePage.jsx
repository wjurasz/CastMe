// src/pages/ProfilePage.jsx - Updated with proper favorites API + toasts + loaders (strict-mode toast guard)
...
export default function ProfilePage() {
  ...
  // Guard przed podwójnym toastem w StrictMode
  const toastShownRef = useRef(false);
  useEffect(() => {
    if (location.state?.profileUpdated && !toastShownRef.current) {
      toastShownRef.current = true;
      show("Profil został zaktualizowany.", "success");
      // czyścimy state, by toast nie wracał po odświeżeniu
      navigate(location.pathname, { replace: true, state: {} });
    }
  }, [location.state, navigate, location.pathname, show]);
  ...
  const toggleFavorite = useCallback(async () => {
    ...
    try {
      if (isFavorite) {
        ...
        setIsFavorite(false);
        show("Usunięto z ulubionych.", "success");
      } else {
        await addFavorite(userId, accessToken);
        setIsFavorite(true);
        show("Dodano do ulubionych.", "success");
      }
    } catch (err) {
      console.error("Favorite toggle error:", err);
      show(err?.message || "Błąd zmiany statusu ulubionych", "error");
    } finally {
      setFavoriteLoading(false);
    }
  }, [
  ...
  // Fullscreen loader podczas ładowania profilu
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="flex items-center gap-3 text-[#2b2628]">
          <span className="inline-block h-8 w-8 rounded-full border-4 border-[#EA1A62] border-t-transparent animate-spin" />
          <span className="font-medium">Ładowanie profilu…</span>
        </div>
      </div>
    );
  }
  ...
  if (!selectedUser)
    return <div className="text-center py-10">Nie znaleziono profilu</div>;

  const age = selectedUser.dateOfBirth
    ? Math.floor(
        (new Date().getTime() - new Date(selectedUser.dateOfBirth).getTime()) /
          31557600000
      )
    : null;
  const locationLabel =
    selectedUser.city && selectedUser.country
      ...
      : null;
  const mainPhoto =
    selectedUser.photos?.find((p) => p.isMain) || selectedUser.photos?.[0];

  const dynamicInfoFields = [
    { label: "Wiek", key: "age", value: age },
    { label: "Lokalizacja", key: "location", value: locationLabel },
    {
    ...
    { label: "Telefon", key: "phone", value: selectedUser.phone },
    {
      label: "Płeć",
      key: "gender",
      value: formatField("gender", selectedUser.gender),
    },
    {
      label: "Wzrost",
      key: "height",
      value: formatField("height", selectedUser.height),
    },
    {
      label: "Waga",
      key: "weight",
      value: formatField("weight", selectedUser.weight),
    },
    { label: "Kolor włosów", key: "hairColor", value: selectedUser.hairColor },
    {
      label: "Rozmiar ubrań",
      key: "clothingSize",
      value: selectedUser.clothingSize,
    },
  ...
  return (
    <div className="bg-white text-[#2b2628] min-h-screen p-4 sm:p-6 md:p-10">
      <div className="max-w-6xl mx-auto">
        ...
        <Card className="p-6 mb-8 text-center relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mt-6 text-center sm:text-center justify-end">
            {dynamicInfoFields.map(({ label, key, value }) => (
              <p
                key={key}
                className="flex items-left justify-start sm:justify-left gap-1"
              >
                {fieldIcons[key] || null}
              ...
              <Button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-full transition-colors ${
                  isFavorite
                    ...
                    : "bg-[#EA1A62] text-black hover:bg-gray-300"
                } disabled:opacity-50`}
              >
                {favoriteLoading && (
          ...
          {allPhotos?.length > 0 ? (
            <>
                  ...
                  .map((photo, idx) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          openModal(idx + currentPage * photosPerPage)
                        }
                      >
                        <img
              ...
              <div className="flex justify-center mt-4 gap-2">
                ...
                <button
                  disabled={
                    (currentPage + 1) * photosPerPage >= allPhotos.length
                  }
        ...
        <Card className="p-6">
          <h2 className="text-2xl font-semibold mb-4 flex items-center">
            <Briefcase size={24} className="mr-2 text-[#EA1A62]" />{" "}
            Doświadczenie
          </h2>
          {selectedUser.experiences?.length > 0 ? (
            selectedUser.experiences.map((exp) => (
              <div
                key={exp.id}
                className="border-l-4 border-[#EA1A62] pl-4 py-2 mb-4"
              >
